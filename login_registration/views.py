...
class CollegeRegistrationView(APIView):
    ...
    def post(self, request):
        ...
        if User.objects.filter(username=request.data['email']).exists() or CollegeRegistration.objects.filter(college_email=request.data['college_email']).exists():
            data['message'] = "error"
            ...
            if user_serializer.is_valid() and college_serializer.is_valid():
                ...
                data['redirect_url'] = "/login"
                data['success'] = "Successfully Registered. You can now login."
            else:
                data['message'] = "error"
                data['error'] = 'Please fill your form correctly !!'
        return Response(data)
...
class UserRegistrationView(APIView):
    ...
    def get(self, request):
        data = {
            'user_list': Profile.objects.filter(college=request.user.profile.college, login_allow=True)
        }
        return Response(data)
