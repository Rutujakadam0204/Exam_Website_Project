...
def paymentview(request, token):
    if request.method == "GET":
        ...
        if CollegeRegistration.objects.filter(college_email=college_email, payment=True).exists():
            return redirect('/accounting/payment-history')
        else:
            ...
            context['student_limit'] = student_limit
            context["service_provider"] = "payu_paisa"
            context['payment_gateway'] = 'payu'
            context["furl"] = 'http://'+current_domain+'/accounting/payment_handler/fail/' + token
            context["surl"] = 'http://'+current_domain+'/accounting/payment_handler/pass/' + token
        return render(request, 'payment.html', context)
    
@csrf_exempt
def payment_handler(request, status, token):
    data = {}
    student_limit = float(request.POST['net_amount_debit']) / float(PackageSetting.objects.all().latest('id').price_per_student)
    ...
    if status == 'fail':
        ...
        college.save()
        invoice = Invoice.objects.create(college=college, price=float(request.POST['net_amount_debit']), student_limit=student_limit, expiry=datetime.now()+timedelta(seconds=31536000))
        invoice.save()
